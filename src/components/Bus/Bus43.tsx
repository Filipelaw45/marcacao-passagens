...
export function Bus43({ trip }: Bus43Props) {
  const upperDeckFirstColumn = [
    1, 2, 4, 5, 7, 8, 9, 10, 11, 12, 14, 15, 17, 18, 20, 21, 23, 24, 26, 27, 29,
    30,
  ];
        ...
        <section className='flex flex-col items-center relative'>
          <section className='max-w-[16rem] h-[35rem] ml-5 mr-5 mt-5 mb-2 flex rounded-t-3xl p-2 bg-neutral-300'>
            ...
            <section className='h-full flex-1 flex flex-wrap justify-center'>
              {Array.from({ length: upperDeckFirstColumn.length }).map(
                (_, index) => {
            ...
            <section className='h-full flex-1 bg flex flex-col flex-wrap gap-[1.3%] mt-1'>
              {Array.from({ length: upperDeckSecondColumn.length }).map(
                (_, index) => {
                  const isPreferential = preferentialSeats.includes(
                    upperDeckSecondColumn[index]
                  );
                  const passenger = trip.passengers.find(
              ...
              {Array.from({ length: upperDeckSecondColumn2.length }).map(
                (_, index) => {
                  const isPreferential = preferentialSeats.includes(
                    upperDeckSecondColumn2[index]
                  );
                  const passenger = trip.passengers.find(
          ...
          <section className='max-w-[16rem] h-[35rem] ml-5 mr-5 mt-5 mb-2 flex rounded-t-3xl p-2 bg-neutral-300 flex-col'>
            ...
            <div className='flex h-[40%] m-2 pr-4 gap-5'>
              ...
              <section className='h-full flex flex-wrap justify-center'>
                {Array.from({ length: lowerDeckFirstColumn.length }).map(
                  (_, index) => {
                    const isPreferential = preferentialSeats.includes(
                      lowerDeckFirstColumn[index]
                    );
                    const passenger = trip.passengers.find(
              ...
              <section className='h-full flex flex-row-reverse flex-wrap justify-center'>
                {Array.from({ length: lowerDeckSecondColumn.length }).map(
                  (_, index) => {
                    const isPreferential = preferentialSeats.includes(
                      lowerDeckSecondColumn[index]
                    );
                    const passenger = trip.passengers.find(
